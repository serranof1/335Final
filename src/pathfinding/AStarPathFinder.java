...
public class AStarPathFinder implements PathFinder {
	...
	public Path findPath(Mover mover, int sx, int sy, int tx, int ty) {
		...
		System.out.println(map.getTile(tx, ty).canMove());
		if (!map.getTile(tx, ty).canMove()) {
			return null;
		}
		...
		int maxDepth = 0;
		while ((maxDepth < maxSearchDistance) && (open.getSize() != 0)) {
	...
	protected boolean isValidLocation(Mover mover, int sx, int sy, int x, int y) {
		boolean invalid = (x < 0) || (y < 0) || (x >= map.getSize()) || (y >= map.getSize());
		
		if ((!invalid) && ((sx != x) || (sy != y))) {
			invalid = !map.getTile(x, y).canMove();
		}
