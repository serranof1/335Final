...
/**
 * RepairTask is a {@link Task} that assigns a {@link Drone} to find a
 * {@link Engineering} and repair.
 * 
 * @author Team Rosetta
 *
 */
public class RepairTask extends Task {
	...
	/**
	 * @param map
	 *            - The {@link Map} on which to execute. The {@link Drone}'s
	 *            needs are decremented and the drone is repaired. If it is not
	 *            at a {@link Engineering} it is assigned a {@link MoveTask}.
	 */
	@Override
	public boolean execute(Map map) {
		...
		if (drone.getCurrentTile() == goal) {
			...
			drone.getTaskList().push(
					new MoveTask(drone, building.getEmptyTile(), false));
