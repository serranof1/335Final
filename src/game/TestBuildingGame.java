...
public class TestBuildingGame {
	...
	private void debugMethod() {
		...
		// testMove.getTaskList().push(new MoveTask(testMove, map.getTile(10,
		// 10), false));
		// testMove2.getTaskList().push(new MoveTask(testMove2, map.getTile(10,
		// 10), false));
	...
	private void checkNeeds(Drone drone) {
		if (drone.getRepair() < 30) {
			...
			System.out.println("repairAt location: "
					+ repairAt.getLocation().getX() + " "
					+ repairAt.getLocation().getY());
			if (!drone.isRepairing()) {
				drone.getTaskList()
						.push(new RepairTask(drone, repairAt, repairAt
								.getEmptyTile()));
				drone.toggleRepair();
			}
		}
		if (drone.getGas() < 50) {
			Building fillAt = map.findNearest(drone.getCurrentTile(),
					BuildingEnum.METHANEPLANT);
			if (!drone.isFilling()) {
				drone.getTaskList().push(
						new MethaneTask(drone, fillAt, fillAt.getEmptyTile()));
				drone.toggleFilling();
			}
		}
		...
		for (int i = 0; i < allDrones.size(); i++) {
			for (int j = 0; j < allDrones.get(i).size(); j++) {
				...
				System.out.println("Tasks: "
						+ allDrones.get(i).get(j).getTaskList());
			}
		}
		System.out
				.println("**************************************************************");
